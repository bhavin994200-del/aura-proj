...
export default function SwingTradesModule() {
  ...
  // 🔥 પેજ ઓપન થાય એટલે તરત જ સ્ટેટિક સ્કેનરની જેમ આખું 225+ સ્ટોક્સનું લિસ્ટ સ્કેન કરશે
  useEffect(() => {
    fetchAllStaticData();
  }, []);
  ...
  const fetchAllStaticData = async () => {
    ...
    try {
      const res = await fetch('https://aura-proj.onrender.com/scan-static-pivot', {
        ...
        body: JSON.stringify({ symbols: fullFnoList }) // આખું 225+ F&O લિસ્ટ મોકલશે
      });
  ...
  // 🔥 1. Weekly Buy Setups (પ્યોર સ્ટેટિક વીકલી સપોર્ટ બેઝ્ડ)
  ...
  // 🔥 2. Weekly Sell Setups (પ્યોર સ્ટેટિક વીકલી રેઝિસ્ટન્સ બેઝ્ડ)
  ...
  // 🔥 3. Monthly Buy Setups (પ્યોર સ્ટેટિક મંથલી સપોર્ટ બેઝ્ડ)
  ...
  // 🔥 4. Monthly Sell Setups (પ્યોર સ્ટેટિક મંથલી રેઝિસ્ટન્સ બેઝ્ડ)
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '15px' }}>
        <div>
          <h2 style={{ color: '#18181b', margin: '0 0 5px 0' }}>🚀 Static Scanned Swing Trades</h2>
          <p style={{ color: '#52525b', fontSize: '14px', marginTop: '0' }}>
            {isLoading ? 'Scanning all 225+ stocks from server...' : `Loaded Scanner Database: ${liveMarketStocks.length} Stocks`}
          </p>
        </div>
      ...
      <div style={{ maxHeight: '550px', overflowY: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '14px' }}>
          ...
          <tbody>
            {data.length === 0 ? (
              <tr>
                <td colSpan="7" style={{ textAlign: 'center', padding: '30px', color: '#71717a' }}>No matching swing setups found. Click "Scan Static Levels" above!</td>
              </tr>
            ) : (
              data.map((item, idx) => (
